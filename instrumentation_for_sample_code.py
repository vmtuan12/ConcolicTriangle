...
# instrumentation code opening/closing function
def make_instrumentation_function(line, statement, space, isOpening):
    result = ''
    if (isOpening):
        result = f'mark("statement${statement}#line-of-blockin-function${line}#openning-function$true")\n'
    else:
        result = f'mark("statement${statement}#line-of-blockin-function${line}")\n'
    return result.rjust(space + len(result), ' ')
...
regex_for = r"^\s*for *[a-zA-Z]{1}[\w, _]* *in *((range[(].*[)])|([a-zA-Z]{1}[\w, _]*)) *:$"
...
if_space_dict = dict()
if_line_dict = dict()
function_instru_code_space = 0
function_instru_code_line = 0
code_dicts = []
...
for index in range(len(code_dicts)):
    ...
    match_for = re.search(regex_for, line)

    if match_def:
        function_instru_code_line = index + 1 + 1
        function_instru_code_space = spaces + 4
        line += make_instrumentation_function(function_instru_code_line, 'function-start', function_instru_code_space, True)
